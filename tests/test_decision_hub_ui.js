...
assert(js.includes("prediction-engine.html?horizon=UP_45D"));
assert(js.includes("index.html?view=TRAINING"));
assert(js.includes("showDecisionFromStatus('conflict'"));
assert(js.includes("showShadowFromStatus('medium')"));
assert(js.includes('點一下查看'));
...
assert(js.includes('factor_explanation'));
assert(js.includes('本檔排名原因'));
assert(js.includes('不代表因果或保證漲跌'));
...
const expectedDecisionCount=Object.values(report.summary.by_market).reduce((total,count)=>total+count,0);
assert.strictEqual(report.summary.decision_count,expectedDecisionCount);
...
assert.strictEqual(decisions.length,report.summary.decision_count);
