...
assert.match(html, /UP_60D/);
assert.match(html, /UP_126D/);
assert.match(html, /產業位置、生命週期、當時證據、風險、日期與來源/);
assert.match(html, /正式 V6、原本 5 日／60 日紀錄/);
assert.match(js, /reports\/prediction_engine\.json/);
assert.match(html, /prediction_engine\.js\?v=3/);
assert.match(js, /REPORT\.data_files/);
assert.match(js, /本次答案模型/);
assert.match(js, /本檔排名原因/);
assert.match(js, /factor_discovery/);
assert.match(html, /哪些數據與上漲有關/);
assert.match(html, /不代表因果或保證上漲/);
assert.match(js, /industry_lifecycle/);
assert.match(js, /產業階段/);
assert.match(js, /七週期答案/);
