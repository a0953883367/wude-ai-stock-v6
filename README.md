...
隔日預測 V2 只在市場正式收盤後保存快照：台股使用晚報、美股使用次日早報，
並以資料源提供的正式交易日與調整後收盤價對齊。同市場、同股票、同交易日只
保存一次；舊版以報告時間推算交易日的紀錄不再參與統計或分數校正。

`performance.json` 會保存台股、美股與 ETF 的 1／5／10／20 個交易日結果，
並以十個透明候選模型進行 shadow test。報表同時顯示方向命中率、樣本數、
訊號涵蓋率、方向調整後平均報酬與最差報酬。累積未達 60 個交易日且 200 筆
共識訊號前，結果一律不影響 AI 分數；不得用程式測試通過率代替投資績效。
