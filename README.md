...
或一般方向校準，並提出研究候選；20 日只做初評，60 日後仍須人工決定，不能
回寫 V6、改權重、自動 Merge 或下單。
...
成熟門檻。可訓練模型建立影子挑戰者，規則模型只校準門檻，監控與資料守門員只
監控而不學股價方向。登錄完成代表已納入治理，不代表樣本已成熟或可以升級。
...
六個週期拆成 24 份按需載入檔案，每個索引或分檔各有 300 KB 防呆上限，沒有
