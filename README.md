...
`trade_plan_shadow.py` 只讀中央 AI 已完成的 `decision_hub` 報表，整理每檔股票的
買進有效交易日、買進區、不追價上限、停損、第一／第二目標、分批賣出比例與最長
持有週期。頁面入口位於中央 AI 決策中樞的「交易計畫」。

計畫分為 1～5 日、45 日、約 6 個月三個週期；買進期限到期、正式收盤資料更新、
風險新增或買進區失效時必須重算。此模組固定為影子研究層，不修改正式 V6、正式
排名或正式權重，也不呼叫任何券商下單 API。

`trade_plan_validation.py` 另建立完全前向的驗證帳本：先保存當下交易計畫，再只用後續完成交易日的官方收盤價判定是否進入買進區、是否到達目標或跌破停損。相同交易日不得當成結果，盤中碰價也不會自行假設成交。這份驗證只用來評估交易計畫品質，不會回寫正式 V6 或觸發下單。
...
`agent_control.py` 將任務分派給股票影子、WT 螺絲電商或包裝創業
三個獨立 Agent。共用層只保存路由、權限、API 用量、核准狀態與稽核摘要，
不建立跨事業共用資料庫，也不保存任務的敏感內容。

安全、可逆的分析、草稿、模擬與影子驗證可以執行；正式 V6 權重、正式排名、
券商下單、付款、對外寄送／發布及刪除歷史均保持鎖定。
WT 與包裝創業在正式資料來源接妥前只以測試模式運作。

`agent_runtime.py` 會為三個 Agent 各執行一條不接外部系統的安全驗收，並同時
確認股票下單、WT 對外上架及包裝創業付款仍會停在授權閘門。
公開報告只包含任務摘要、雜湊、結果與授權狀態，不保存客戶、價格或任務內容。
