...
`agent_control.py` 將任務分派給股票影子、至盈公司、WT 螺絲電商或包裝創業
四個獨立 Agent。共用層只保存路由、權限、API 用量、核准狀態與稽核摘要，
不建立跨事業共用資料庫，也不保存任務的敏感內容。

安全、可逆的分析、草稿、模擬與影子驗證可以執行；正式 V6 權重、正式排名、
券商下單、付款、對外寄送／發布、ERP／PLC／設備寫入及刪除歷史均保持鎖定。
至盈公司連線仍須公司同意；WT 與包裝創業在正式資料來源接妥前只以測試模式運作。

`agent_runtime.py` 會為四個 Agent 各執行一條不接外部系統的安全驗收，並同時
確認股票下單、公司 ERP 寫入、WT 對外上架及包裝創業付款仍會停在授權閘門。
